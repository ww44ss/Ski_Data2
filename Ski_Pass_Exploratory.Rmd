...
```{r "try a season hack"}
run_list$season<-rep("00-00", dim(run_list)[1])

run_list<-as.matrix(run_list)

season_hacker<-function(x)
        {
        d<-as.Date(as.character(x$ski_date))
...
```{r}
for (i in 1:dim(run_list)[1]){
        run_list$season<-season_hacker(run_list[i,])
}
...
library(ggplot2)
p<-ggplot(run_list[1:370,], aes(x=ski_date, y=Var1, size=Freq))
p<-p + geom_point()
...
```{r, fig.width=10, fig.height=4}
library(ggplot2)
p<-ggplot(run_list[370:740,], aes(x=ski_date, y=Var1, size=Freq))
p<-p + geom_point()
p<-p + theme(axis.text.x=element_text(angle=60, size=10, vjust=0.5))
print(p)
