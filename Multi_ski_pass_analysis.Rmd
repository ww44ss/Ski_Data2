...
#Summary  
This analysis looks at skier data in the form of chairlift rides for individual ski pass holders. A few sample skier "days" for multiple seasons (05-06 to 14-15) are used to draw general conclusions about the capability of modeling in enhancing prediction of skier visits and chair lift rides.  
...
2. Area-managament and Area-users: Models can detect fraudulent pass use, protecting purchasers and area managment from fraud. 
...
```{r, "skiers per day series plot1314",echo=FALSE, results='asis', fig.align='center', fig.height=3, fig.width=8}
    
    
     p2<-ggplot(skier_day_count_13_14, aes(x=as.Date(all_ski_days, "%Y-%m-%d"), y=normal_Freq, fill=wday))
    p2<-p2+geom_bar(stat="identity")
    p2 <- p2 + ggtitle("Skiers Per Day 13-14")
    p2 <- p2 + xlab("date")
    p2 <- p2 + ylab("normalized frequency")
