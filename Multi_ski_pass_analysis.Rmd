...
for (pass in sample(valid_pass, 8)) {
    ...
    ##xxx<-sample(valid_pass,8)  # for debug
    ##pass<-xxx[2]
    ...
    for (days in ski_days) {
        ...
        ss$ski_speed<-(ss$base_elev+ss$vertical_feet-ss$elev)/(as.numeric(ss$ski_time)-ss$ride_time)
...
A note here. I've filtered out several items:
ski times that are below the chairlift ride time result in "negative" speeds and are anomalous. It will be worth looking at these in more detail, because they appear to occur for some users and not others. In some cases it appears that riders may have checked in to, say Summit, but then decided to ski back down to Sunrise. In others lap times of long chairs, such as Northwest, just appear to be shorter than the chair ride time.  
