...
pass_sample<-sample(valid_pass, 10)
...
```{r, "ski speed means", echo=FALSE}
    
    ## Use above method to calculate ski descent time and vertical / time
    ## the general approach will be to calculate first for each run (chair ride),
    ##      a start time, 
    ##      a start elevation, an end time, and an end elevation. 
    ##
...
# big_skier_record<-NULL
# 
# ## for this calc sample a large number of passes
# pass_sample<-sample(valid_pass, 100)  
# 
# for (pass in pass_sample) {
#     ## read in the pass data
#     
...
#     
#         ## rename to something in English
#         levels(ski_runs$holiday) <- c("weekday", "holiday/weekend")
#    
#     ## add lookup data on vase vert and chair ride duration
#     i<-cbind(match(ski_runs$chair, rownames(look_up)))
#     
#     ## add base elevationa and ride time columns to the data frame
#     ski_runs<- cbind(ride_time=look_up[i,1], base_elev=look_up[i,2], ski_runs)
# 
#     ##get rid of junk columns
#     ski_runs$vertical_meters<-NULL
...
#     ski_runs$time_end<-as.character(ski_runs$time)
#     
#     ## do by holiday and non-holiday
#     ski_days<-unique(ski_runs$holiday)
#     
#     for (days in ski_days) {
#         
#         ## get runs for specific day
...
```{r "skier probs", echo=FALSE, results="asis"}
    ...
    print(xtable(probs), type='html', comment=FALSE, include.rownames=FALSE, html.table.attributes='border="2" align="center" ' )
...
for (pass in pass_sample){
    ...
    for (date in dates){
        ...
        if (length(ski_day$chair_p) < 6 ) {day_prob <- 0}
...
## Select only a few passes to keep sane
    ...
    prob_junk<-ski_day_prob_accumulator[
        (ski_day_prob_accumulator$pass == "28cd6f49" |
